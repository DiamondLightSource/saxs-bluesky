...
def get_saxs_beamline() -> str:
    ...
    if beamline is None:
        raise ValueError("No beamline set")
...
def authenticate(beamline: str | None = None):
    ...
    subprocess.run([f"blueapi -c {blueapi_config_path} login"], shell=True)
...
def open_scripting(beamline: str | None = None):
    ...
    root_path = Path(saxs_bluesky.__file__).parent.parent.parent.parent
    example_path = os.path.join(
        root_path, "saxs-bluesky", "user_scripts", f"{beamline}"
    )
